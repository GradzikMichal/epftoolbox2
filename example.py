...
if __name__ == "__main__":
    pipeline = DataPipeline("2025-06-01","2025-08-20")
    ...
    data=pipeline.execute("data_cache")
    
    dummies = ['is_monday', 'is_tuesday', 'is_wednesday', 'is_thursday', 'is_friday', 'is_saturday', 'is_sunday','is_holiday']
    
    pipeline = EvaluatorPipeline(1, data, "2025-07-15", "2025-08-01")
    
    pipeline.add_model(OLSModel(['load','load_d-1','load_d-7'], dummies, name="AR", target='load', trainingWindow=10, saveToFile="ar.json"))
    pipeline.add_model(LassoModel(['load','load_d-1','load_d-7','temperature_2m_d+1','cloud_cover_d+1','precipitation_d+1'], dummies, name="ARX", target='load', trainingWindow=10, saveToFile="arx.json"))
